 ...
 * 
 * @param inputObj
 *            The input object want to contain date.
 * @param dateFormatStyle
 *            Default Date Formatter is "yyyy-MM-dd", you could use your own
 *            defined format.
 * @param beginDate
 *            Default value is 1990-01-01
 * @param endDate
 *            Default value is 2020-01-01
 * @param lang
 *            0(English)|1(Chinese) Default Language is 0(English).
 */
 ...
 *            The input object want to contain date.
 * @param dateFormatStyle
 *            Default Date Formatter is "yyyy-MM", you could use your own
 *            defined format.
 * @param beginDate
 *            Default value is 1990-01
 * @param endDate
 *            Default value is 2020-01
 * @param lang
 *            0(English)|1(Chinese) Default Language is 0(English).
 */
function setmonth(inputObj, dateFormatStyle, beginDate, endDate, lang) {
	...
	new Calendar(inputObj, dateFormatStyle, beginDate, endDate, lang, "m")
			.show();
}
...
/**
 * //Classic Style Calendar.prototype.style = function(){ var strStyle = "<style
 * type='text/css'>"; strStyle += ".calendar {font-size:12px;
 * margin:0;padding:0px;border:1px solid #397EAE;}"; strStyle += ".calendar ul
 * {list-style-type:none; margin:0; padding:0;vertical-align:middle;}"; strStyle +=
 * ".calendar li {float:left;}.calendar b{font-weight:bold;}"; strStyle +=
 * ".calendar .day { background-color:#EDF5FF; height:20px;}"; strStyle +=
 * ".calendar .day li,.calendar .date li{ float:left; width:14.13%; height:20px;
 * line-height:20px; text-align:center}"; strStyle += ".calendar .month li{
 * float:left; width:24.8%; height:20px; line-height:20px; text-align:center}";
 * strStyle += ".calendar li a{ text-decoration:none; font-family:Tahoma;
 * font-size:11px; color:#333}"; strStyle += ".calendar li:hover
 * {cursor:pointer;color:#f30; text-decoration:none;background-color:#E8F2FE;}";
 * strStyle += ".calendarlihover
 * {color:#f30;text-decoration:none;background-color:#E8F2FE;}"; strStyle +=
 * ".calendar li a.hasArticle {font-weight:bold; color:#f60 !important}";
 * strStyle += ".lastMonthDate, .nextMonthDate {color:#bbb;font-size:11px}";
 * strStyle += ".selectThisYear, .selectThisMonth{text-decoration:none;
 * margin:0px; color:#000; font-weight:bold}"; strStyle += ".calendar
 * .LastMonth, .calendar .NextMonth{text-decoration:none; color:#000;
 * font-size:18px; font-weight:bold; line-height:16px;}"; strStyle +=
 * ".calendarTitle
 * {text-align:center;height:20px;line-height:20px;clear:both;width:100%;}";
 * strStyle += ".calendarTitle .mark{text-decoration:
 * none;color:#000;font-family:Tahoma;font-size:18px;font-weight:normal;line-height:
 * 16px;}"; strStyle += ".today{ background-color:#ffffaa;border:1px solid
 * #f60;padding:0 1px;}"; strStyle += ".today a { color:#f30; }"; strStyle +=
 * ".calendarBottom
 * {text-align:center;height:20px;line-height:20px;clear:both;width:100%;border-top:1px
 * solid #ddd;}"; strStyle += ".calendarBottom
 * li{float:left;height:20px;line-height:20px;font-weight:bold;text-align:center;}";
 * strStyle += "</style>"; return strStyle; }
 */
...
String.prototype.toDate = function(format) {
	...
	var pattern = format.replace("yyyy", "(\\~1{4})")
			.replace("yy", "(\\~1{2})").replace("MM", "(\\~1{2})").replace("M",
					"(\\~1{1,2})").replace("dd", "(\\~1{2})").replace("d",
					"(\\~1{1,2})").replace(/~1/g, "d");
...
Date.prototype.dateAdd = function(interval, number) {
	switch (interval) {
	case "y":
		return new Date(this.getFullYear() + number, this.getMonth(), this
				.getDate());
		break;
	case "m":
		return new Date(this.getFullYear(), this.getMonth() + number,
				checkDate(this.getFullYear(), this.getMonth() + number, this
						.getDate()));
		break;
	case "d":
		...
		break;
	case "w":
		return new Date(this.getFullYear(), this.getMonth(), 7 * number
				+ this.getDate());
		break;
	}
}
...
/**
 * Calendar language pack default support english and chinese,if you want to add
 * some other language, please extend it.
 */
Calendar.language = {
	"title" : [ [ "", "" ], [ "年", "月" ] ],
	"months" : [
...
Calendar.prototype.draw = function() {
	...
	if ("d" == this.type) {
		...
		lis += "<li style='width:30%;'><span id='selectThisYear' class='selectThisYear'>"
				+ this.date.getFullYear()
				+ "</span>"
				+ Calendar.language["title"][this.lang][0] + "</li>";
		if (0 == this.lang) {
			lis += "<li style='width:20%;'><span id='selectThisMonth' class='selectThisMonth'>"
					+ Calendar.language["months"][this.lang][this.date
							.getMonth()] + "</span></li>";
		} else {
			lis += "<li style='width:20%;'><span id='selectThisMonth' class='selectThisMonth'>"
					+ (this.date.getMonth() + 1)
					+ "</span>"
					+ Calendar.language["title"][this.lang][1] + "</li>";
		}
		...
		for (var i = 0; i < Calendar.language.weeks[this.lang].length; i++) {
			lis += "<li title='" + Calendar.language.weekday[i] + "'>"
					+ Calendar.language.weeks[this.lang][i] + "</li>";
		}
		lis += "</ul><ul class='date' id='thisMonthDate'>";
		var lastMonthLis = "";
		for (var i = 0; i < lastMonthDate; i++) { // Last Month's Date
			// alert(lastMonthDate + "," + lastMonthEndDate);
			lastMonthLis = "<li class='lastMonthDate'>" + lastMonthEndDate
					+ "</li>" + lastMonthLis;
			lastMonthEndDate--;
		}
		lis += lastMonthLis;
		...
		lis += "<li id='emptyCalendar' style='width:27%;' title='Clear'>"
				+ Calendar.language.clear[this.lang] + "</li>";
		lis += "<li id='selectCurrent' style='width:45%;' title='Today'>"
				+ Calendar.language.today[this.lang][0] + "</li>";
	} else {
		lis += "<li style='width:74%;'><span id='selectThisYear' class='selectThisYear'>"
				+ this.date.getFullYear()
				+ "</span>"
				+ Calendar.language["title"][this.lang][0] + "</li>";
		lis += "<li id='NextYear' class='mark' style='width:12%;' title='Next Year'>&raquo;</li></ul></div>";
		...
		for (var i = 1; i <= 12; i++) {
			var currentDate = (this.year + "-" + (i > 9 ? i : "0" + i))
					.toDate("yyyy-MM");
			if (currentDate < this.beginDate || currentDate > this.endDate) {
				lis += "<li class='lastMonthDate'>" + i + "</li>";
				continue;
			}
			lis += "<li class='thisMonth' title='" + this.year + "-"
					+ (i > 9 ? i : "0" + i)
					+ "-01'><a href='javascript:void(0);'";
			if ((this.year + "-" + (i > 9 ? i : "0" + i)) == (this.date)
					.format("yyyy-MM")) {
				lis += " class='today' ";
			}
			lis += ">" + i + "</a></li>";
		...
		lis += "<li id='emptyCalendar' style='width:27%;' title='Clear'>"
				+ Calendar.language.clear[this.lang] + "</li>";
		lis += "<li id='selectCurrent' style='width:45%;' title='Current Month'>"
				+ Calendar.language.today[this.lang][1] + "</li>";
	}

	lis += "<li id='closeCalendar' style='width:27%;' title='Close'>"
			+ Calendar.language.close[this.lang] + "</li>";
...
Calendar.prototype.selectThisMonth = function() {
	...
	for (var i = 1; i <= endMonth; i++) {
		selectStr += "<option value='" + i + "'>"
				+ Calendar.language["months"][this.lang][i - 1] + "</option>";
	}
	selectStr += "</select>";
	spanObj.innerHTML = selectStr;
...
Calendar.prototype.getElementsByTagName = function(tagName) {
	...
	if (this.document.all)
		return this.document.all.tags(tagName);
}
...
Calendar.prototype.setDisplayStyle = function(tagName, style) {
	...
	for (var i = 0; i < tags.length; i++) {
		if (tagName.toLowerCase() == "select"
				&& (tags[i].name == "calendarYear" || tags[i].name == "calendarMonth")) {
			continue;
		}
...
document
		.write('<div id="calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;font-size:12px;width:20px;">');
document
		.write("<iframe id='calendarIframe' scrolling='no' frameborder='0' width='100%' height='100%'></iframe></div>");